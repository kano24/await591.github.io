...
(function(w) {
    ...
    function shareText(score) {
        ...
        if (mode === MODE_ENDLESS) {
            if (score <= 5) return '诶，怎么都不喝啊？';
            if (score <= 8) return '这喝汤多是一件美逝啊！';
            if (score <= 10)  return '不咸不淡，味道真是好寄了！';
            if (score <= 15) return '他奶奶滴，为什么不喝！';
            return '不喝？不喝也别想活着！';
        } else {
    ...
    function initSetting() {
        ...
        document.getElementsByTagName("title")[0].innerText = cookie("title") ? cookie("title") : "吃掉催逝员";
        if (cookie("keyboard")) {
